...
public:
    ...
    Matrix Backward(Matrix& output_grad)
    {
        ...
        //// Code start ////
        Matrix stored_inputT = this->stored_input;
        this->stored_input.tpose(stored_inputT);

        this->weight_grad = stored_inputT * output_grad;

        Matrix weightT = this->weight;
        this->weight.tpose(weightT);

        return output_grad * weightT;
        //// Code end   ////
    }
};
...
public:
    ...
    {
        /*
         *  The input matrix has the dimension [#samples, #features].
         *  The output matrix has the same dimension as the input.
        ...
        for (int i = 0; i < input.m * input.n; i++) {   // loop through all elements
            output.data[i] = max((double)0, (double)input.data[i]);
        }
        stored_input = input;
        return output;
        //// Code end   ////
...
public:
    ...
    double Forward(Matrix& pred, const Matrix& truth)
    {
        ...
        return L2(stored_data) * ((double)1 / (double)pred.m);
        //// Code end   ////
    }
    ...
    Matrix Backward()
    {
         ...
         */
         //// Code start ////
        return stored_data * (2.0 / (double)stored_data.m);
         //// Code end   ////
    }
};
...
public:
    ...
    Network(const vector<pair<int, int>>& feature_sizes)
    {
       ...
       */
       ...
       //// Code start ////
        linear_layers.resize(feature_sizes.size());
        activation_layers.resize(feature_sizes.size()-1);
            for (int i = 0; i < feature_sizes.size(); i++) {
                LinearLayer A = LinearLayer(feature_sizes[i].first, feature_sizes[i].second);
                linear_layers[i] = A;
            }
       //// Code end   ////
    }
    ...
    Matrix Forward(const Matrix& input)
    {
        ...
        for (int i = 0; i < linear_layers.size() - 1; i++) {
            curr = activation_layers[i].Forward(linear_layers[i].Forward(curr));
        }
        curr = linear_layers[linear_layers.size() - 1].Forward(curr);
        return curr;
         //// Code end   ////
    }
    ...
    Matrix Backward(const Matrix& output_grad)
    {
         ...
         */
         //// Code start ////
        Matrix curr = output_grad;
        for (int i = linear_layers.size() - 1; i > 0; i--) {
            curr = activation_layers[i-1].Backward(linear_layers[i].Backward(curr));
        }
        curr = linear_layers[0].Backward(curr);
        return curr;
         //// Code end   ////
    }
};
...
Matrix Matrix_Slice(const Matrix& A, const int start, const int end)
{
     ...
     */
     //// Code start ////
    Matrix Slice = A;
    Slice = Slice.Block(start, 0, end, A.n);
    return Slice;
     //// Code end   ////
}
...
Matrix One_Hot_Encode(vector<int> labels, int classes = 10)
{
     ...
     */
     //// Code start ////
    Matrix Encode(labels.size(), classes);
...
public:
    ...
    double Train_One_Epoch()
    {
        ...
        for (int i = 1; i < n_loop; i++) {
            ...
            Matrix Encode = One_Hot_Encode(batch_labels, 10);
            Matrix pred = this->net.Forward(batch_data);
            loss += this->loss_function.Forward(pred, Encode);
            Matrix pred_grad = this->loss_function.Backward();
            net.Backward(pred_grad);   //// we do not need the gradient for train_data,but just the parameters.
            for (auto& item : this->net.linear_layers) {
                item.weight -= item.weight_grad * this->learning_rate;
    ...
    void Train()
    {
        for (int i = 0; i < this->max_epoch; i++) {
            ...
            if (accuracy >= 62.5 && learning_rate > .0445) learning_rate = .4345;
            if (accuracy > .73 && learning_rate > .0445) learning_rate = .0445;
            if (accuracy >= .84 && learning_rate > .0145) learning_rate = .01305;
            if (accuracy >= .86 && learning_rate > .0145) learning_rate = .00105;
            //if (accuracy >= .84 && learning_rate > .0145) learning_rate = .0145;
            if (best >= accuracy && best > .60) learning_rate = learning_rate * .956;
            else learning_rate = learning_rate * .96;
        }
    }
